...
static byte_t sim_step()
{
    ...
    cond = cond_holds(cc, (cond_t)ifun);
    ...
    aluB = gen_aluB();
    alufun = gen_alufun();
    vale = compute_alu((alu_t)alufun, aluA, aluB);
    cc_in = cc;
    if (gen_set_cc())
    cc_in = compute_cc((alu_t)alufun, aluA, aluB);
	      ...
	      int argc, char *argv[])
{
    ...
    interp->result = stat_name((stat_t)run_status);
    return TCL_OK;
}
